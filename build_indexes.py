...
from korpsearch.corpus_reader import CORPUS_READERS
from korpsearch.index import Index, KnownLiteral, Template, TemplateLiteral
from korpsearch.index_builder import PIVOT_SELECTORS, SORTER_CHOICES, build_index
...
FILE_SUFFIXES = set(CompressedFileReader.compressors.keys()) | set(
    CORPUS_READERS.keys()
)
...
def main(args: argparse.Namespace) -> None:
    ...
    if not corpus_file.is_file():
        ...
        if len(candidates) > 1:
            raise ValueError(
                f"Too many possible source files: {', '.join(f.name for f in candidates)}"
            )
        if candidates:
    ...
    if args.clean:
        ...
        logging.info("Removed all indexes")
    ...
    if args.corpus_index:
        ...
        try:
            ...
            logging.info("Corpus index already exists")
        except (FileNotFoundError, AssertionError):
            Corpus.build(corpus_dir, corpus_file, args)
            logging.info("Created the corpus index")
    ...
    if args.features or args.templates:
        with Corpus(corpus_id, base_dir=args.base_dir) as corpus:
            ...
            logging.debug(
                f"Creating {len(templates)} indexes: {', '.join(map(str, templates))}"
            )
            built = 0
            ...
            if built == len(templates):
                ...
                logging.info(
                    f"Created {built} new query indexes, "
...
parser = argparse.ArgumentParser(
    description="Build search index(es) for a given corpus"
)
...
parser.add_argument(
    "--clean", action="store_true", help="remove the corpus index and all query indexes"
)
parser.add_argument(
    "--force", action="store_true", help="build indexes even if they exist"
)
parser.add_argument(
    "--corpus-index", "-i", action="store_true", help="build the corpus index"
)
parser.add_argument(
    "--features",
    "-f",
...
parser.add_argument(
    "--sorter",
    "-s",
    choices=SORTER_CHOICES,
    default=SORTER_CHOICES[0],
...
parser.add_argument(
    "--pivot-selector",
    choices=PIVOT_SELECTORS,
    default=next(iter(PIVOT_SELECTORS)),
    help="[only for sorters 'tmpfile' and 'java'] "
...
if __name__ == "__main__":
    ...
    setup_logger(
        "{relativeCreated:8.2f} min {warningname}| {message}",
        timedivider=60 * 1000,
