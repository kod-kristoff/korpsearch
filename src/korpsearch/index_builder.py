...
from korpsearch import sort
from korpsearch.corpus import Corpus
from korpsearch.disk import DiskIntArray
from korpsearch.index import Index, Template, TemplateLiteral, UnaryIndex
from korpsearch.util import binsearch_first, progress_bar
...
def build_unary_index(
    corpus: Corpus, index_path: Path, template: Template, args: Namespace
) -> None:
    ...
    assert not template.literals, (
        f"Cannot build UnaryIndex from templates with literals: {template}"
    )
...
def build_binary_index(
    ...
    def unary_min_frequency(
        unary: UnaryIndex, unary_key: int, cache: dict[int, bool]
    ) -> bool:
    ...
    test_literals = [
        (corpus.tokens[lit.feature].raw(), lit.offset, lit.value)
        for lit in template.literals
    ]

    skipped_instances = 0
    for pos in progress_bar(
        range(len(corpus) - template.max_offset()), desc="Collecting positions"
    ):
        ...
        if (
            ...
            and val2
            and all(
                lfeat[pos + loffset] != lval for (lfeat, loffset, lval) in test_literals
            )
        ):
            if min_freq and not (
                unary_min_frequency(unary1, val1, cache1)
                and unary_min_frequency(unary2, val2, cache2)
            ):
                skipped_instances += 1
            else:
    ...
    collector.finalise(
        index_path, min_frequency=min_freq, skipped_instances=skipped_instances
    )
        ...
        with open(self.path, "r+b") as file:
            with mmap(file.fileno(), 0) as bytes_mmap:
                sort.quicksort(
                    bytes_mmap,
                    rowbytes,
                    pivotselector=PIVOT_SELECTORS.get(
                        self.args.pivot_selector, sort.random_pivot
                    ),
