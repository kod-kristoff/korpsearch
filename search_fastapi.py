...
from korpsearch.search import main_search
from korpsearch.util import setup_logger
...
app.mount(
    f"/{SETTINGS.demo_dir}",
    StaticFiles(directory=SETTINGS.demo_dir),
    name=str(SETTINGS.demo_dir),
)
...
def api_call(
    call: Callable[..., dict[str, Any]], *args: Any, **xargs: Any
) -> APIResult:
    start_time = time()
    try:
        ...
        logging.info(
            f"Completed in {result['time']:.3f} s: {call.__name__}({', '.join(callargs)})"
        )
        return result
...
parser.add_argument(
    ...
    "-d",
    type=Path,
    help=f"directory where to find the corpora (default: {SETTINGS.base_dir})",
)
parser.add_argument(
    "--cache-dir",
    type=Path,
    help=f"directory where to store cache files (default: {SETTINGS.cache_dir})",
)
parser.add_argument(
    "--host", type=str, help=f"host name to use (default: {SETTINGS.host})"
)
parser.add_argument(
    "--port", type=int, help=f"port number to use (default: {SETTINGS.port})"
)
parser.add_argument(
    "--log-level", type=str, help=f"Log-level (default: {SETTINGS.log_level})"
)
parser.add_argument("--log-config", type=str, help="Log config file")
parser.add_argument("--ssl-keyfile", type=str, help="SSL key file")
parser.add_argument("--ssl-certfile", type=str, help="SS certificate file")

parser.add_argument("--no-cache", action="store_true", help="don't use cached queries")
parser.add_argument(
    "--no-diskarray", action="store_true", help="don't use on-disk arrays"
)
parser.add_argument("--no-binary", action="store_true", help="don't use binary indexes")
parser.add_argument(
    "--internal-merge", action="store_true", help="use the internal (slow) merge"
)
parser.add_argument(
    "--no-sentence-breaks", action="store_true", help="don't care about sentence breaks"
)
parser.add_argument(
    "--filter",
    action="store_true",
    help="filter the final results (should not be necessary)",
)
...
if __name__ == "__main__":
    ...
    print(
        f"Open web demo here: http://{SETTINGS.host}:{SETTINGS.port}/{SETTINGS.demo_dir}/index.html"
    )
    uvicorn.run(
        app,
        host=SETTINGS.host,
